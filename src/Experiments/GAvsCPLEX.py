...
class GAvsCPLEX:
    ...
    def buildGAHeader() -> list:
        ...
        return ["Pop Size", "Num Gens", "is 1D Alphas?", "is Optimized Arcs?", "termination", "stagnation",
                            "Init Strategy", "Init Dist", "Init Params", "Selection", "Tourny Size", "Crossover",
                            "CO Rate", "CO Attempts/Gen", "Replacement Strategy", "Mutation", "Mutate Rate",
    ...
    def buildGAData(self) -> list:
        ...
        return [self.geneticPop.populationSize, self.geneticPop.numGenerations, self.geneticPop.isOneDimAlphaTable,
                ...
                self.geneticPop.initializationDistribution, self.geneticPop.initializationParams,
                self.geneticPop.selectionMethod, self.geneticPop.tournamentSize, self.geneticPop.crossoverMethod,
                self.geneticPop.crossoverRate, self.geneticPop.crossoverAttemptsPerGeneration,
                self.geneticPop.replacementStrategy, self.geneticPop.mutationMethod, self.geneticPop.mutationRate,
                self.geneticPop.perArcEdgeMutationRate, self.gaSolution.trueCost, self.geneticRuntimeInSeconds]
    ...
    def buildCPLEXHeaderRow() -> list:
        ...
        return ["CPLEX Obj Val", "CPLEX Runtime (sec)", "Time Limit", "Status", "Status Code", "Gap", "Best Bound"]
    ...
    def buildCPLEXDataRow(self) -> list:
        ...
        return [self.milpCplexSolver.getObjectiveValue(), self.milpCplexSolver.getCplexRuntime(),
                ...
                self.milpCplexSolver.getCplexStatusCode(), self.milpCplexSolver.getGap(),
                self.milpCplexSolver.getBestBound()]
